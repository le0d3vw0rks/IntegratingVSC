...
function msg(overrides: Partial<UIMessage> & { role: UIMessage["role"] }): UIMessage {
  return { id: "1", parts: [], ...overrides };
}
...
test("MessageList renders user messages", () => {
  render(<MessageList messages={[msg({ role: "user", parts: [{ type: "text", text: "Create a button component" }] })]} />);
  expect(screen.getByText("Create a button component")).toBeDefined();
});

test("MessageList renders assistant messages", () => {
  render(<MessageList messages={[msg({ role: "assistant", parts: [{ type: "text", text: "I'll help you create a button component." }] })]} />);
...
test("MessageList shows content for assistant message with content", () => {
  render(
    <MessageList
      messages={[msg({ role: "assistant", parts: [{ type: "text", text: "Generating your component..." }] })]}
      isLoading={true}
    />
  );
...
test("MessageList shows loading state for last assistant message without parts", () => {
  render(<MessageList messages={[msg({ role: "assistant", parts: [] })]} isLoading={true} />);
  expect(screen.getByText("Generating...")).toBeDefined();
});

test("MessageList doesn't show loading state for non-last messages", () => {
  const messages: UIMessage[] = [
    msg({ id: "1", role: "assistant", parts: [{ type: "text", text: "First response" }] }),
    msg({ id: "2", role: "user", parts: [{ type: "text", text: "Another request" }] }),
  ];
...
test("MessageList renders multiple messages in correct order", () => {
  const messages: UIMessage[] = [
    msg({ id: "1", role: "user", parts: [{ type: "text", text: "First user message" }] }),
    msg({ id: "2", role: "assistant", parts: [{ type: "text", text: "First assistant response" }] }),
    msg({ id: "3", role: "user", parts: [{ type: "text", text: "Second user message" }] }),
    msg({ id: "4", role: "assistant", parts: [{ type: "text", text: "Second assistant response" }] }),
  ];
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  const messages: UIMessage[] = [
    msg({ id: "1", role: "user", parts: [{ type: "text", text: "User message" }] }),
    msg({ id: "2", role: "assistant", parts: [{ type: "text", text: "Assistant message" }] }),
  ];
...
test("MessageList handles empty content with parts", () => {
  render(
    <MessageList
      messages={[msg({ role: "assistant", parts: [{ type: "text", text: "This is from parts" }] })]}
    />
  );
...
test("MessageList shows loading for assistant message with empty parts", () => {
  const { container } = render(
    <MessageList messages={[msg({ role: "assistant", parts: [] })]} isLoading={true} />
  );
  const loadingText = container.querySelectorAll(".text-neutral-500");
  const generatingElements = Array.from(loadingText).filter((el) => el.textContent === "Generating...");
