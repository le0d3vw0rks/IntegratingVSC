...
describe("ChatContext", () => {
  ...
  test("initializes with project ID and messages", () => {
    ...
    (useAIChat as any).mockReturnValue({ ...mockUseAIChat, messages: initialMessages });
  ...
  test("tracks anonymous work when no project ID", async () => {
    const mockMessages = [{ id: "1", role: "user" as const, parts: [] }];
    (useAIChat as any).mockReturnValue({ ...mockUseAIChat, messages: mockMessages });
  ...
  test("passes through AI chat functionality", () => {
    (useAIChat as any).mockReturnValue({ ...mockUseAIChat, status: "streaming" });
